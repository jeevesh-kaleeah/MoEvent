<?php 
		...
		if (isset($_GET['event_id'])) {
			...
			$sql = "SELECT * FROM events WHERE id = $event_id AND type = 'cinema'"; 
			$result = mysqli_query($conn, $sql);
		
			if ($result && mysqli_num_rows($result) > 0) {
				$event_cinema = mysqli_fetch_assoc($result);
				
				$backgroundImage_url = $event_cinema['backgroundImage'];
				?>
<!DOCTYPE html>
<html lang="en">
	<head>
		...
		<section class="event-details" style="text-align: center;">
			<!--<div class="eventposter">-->
				<div class="event-img-container">
					<img src="images/<?php echo $event_cinema['backgroundImage']; ?>" >
				</div>
				<h1 class="event-title" style="margin: 20px auto; font-size:30px; color:cyan;"><?php echo $event_cinema['eventname']; ?></h1>
			
				<div class="info-events">
					<ul>
						<li><?php echo $event_cinema['genre']; ?></li>
						<?php
                                $time = new DateTime($event_cinema['time']);
                                echo '<li></i> Time: ' . $time->format('g:i A') . '</p>';
                                ?></li>
						<li><?php echo $event_cinema['location']; ?></li>
					</ul>
					<br>
					<div class="duration-price">
						<p><?php echo $event_cinema['duration']; ?></p>
						<p><?php echo $event_cinema['price']; ?></p>
					</div>
					<div class="line"></div>
					...
					<h3>Storyline</h3>
					<p><?php echo $event_cinema['description']; ?></p>
				</div>
