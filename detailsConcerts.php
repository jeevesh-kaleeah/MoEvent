<?php 
		...
		if (isset($_GET['event_id'])) {
			...
			$sql = "SELECT * FROM events WHERE id = $event_id AND type = 'concerts'"; 
			$result = mysqli_query($conn, $sql);
		
			if ($result && mysqli_num_rows($result) > 0) {
				$event_concerts = mysqli_fetch_assoc($result);
				
				$backgroundImage_url = $event_concerts['backgroundImage'];
				?>
<!DOCTYPE html>
<html lang="en">
	<head>
    ...
    <header id="sticky-header">
        <div class="main-nav">
			...
			<ul class="nav">
				...
				<li><a href="registration.html">Checkout<span></span></a></li>
			</ul>
		</div>
	</header>
		...
		<section class="event-details" style="text-align: center;">
			<!--<div class="eventposter">-->
				<div class="event-img-container">
					<img src="images/<?php echo $event_concerts['backgroundImage']; ?>" >
				</div>
				<h1 class="event-title" style="margin: 20px auto; font-size:30px; color:cyan;"><?php echo $event_concerts['eventname']; ?></h1>
			
				<div class="info-events">
					<ul>
                        <li>Date: <?php echo date('D, M d', strtotime($event_concerts['date'])); ?></li>
                        <li>Time: <?php echo date('g:i A', strtotime($event_concerts['time'])); ?></li>
                        <li><?php echo $event_concerts['location']; ?></li>
					</ul>
					<br>
					<div class="duration-price">
						<p><?php echo $event_concerts['duration']; ?></p>
						<p><?php echo $event_concerts['price']; ?></p>
					</div>
					<div class="line"></div>
					...
					<h3>Storyline</h3>
					<p><?php echo $event_concerts['description']; ?></p>
				</div>
